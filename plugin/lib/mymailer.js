var Gmail = function(localJQuery) {
  ...
  api.get.unread_id = function() {
    var subject = $(".zE td .xS .y6 span").html();
    var time = $(".zE td.xW span").attr("title");

    var emails = api.get.visible_emails();
    for (i = 0; i < emails.length; i++) { 
      if( time == emails[i].time && emails[i].title == subject ){ 
        var id = emails[i].id;
        break;
      }
    }
  ...
  api.tools.parse_view_data = function(view_data) {
    ...
    for(var i=0; i < data.length; i++) {
      ...
      var string = x[7];
      var nstr = string.split('class="');
      var astr = nstr[1];
      var emailclass = astr[0].concat(astr[1]);
